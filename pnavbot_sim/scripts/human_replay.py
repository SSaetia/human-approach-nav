...
class OccupancyGridPGM:
    def __init__(self, map_yaml_path: str):
        ...
        self.w, self.h, self.maxval, self.data = self._load_pgm_p5(pgm_path)
    ...
    def _load_pgm_p5(path: str):
        with open(path, "rb") as f:
            ...
            if tokens[0] != b"P5":
                raise RuntimeError(f"Only P5 PGM supported, got {tokens[0]} in {path}")

            w = int(tokens[1]); h = int(tokens[2]); maxval = int(tokens[3])
            data = f.read(w * h)
            if len(data) != w * h:
...
class HumanReplay(Node):
    def __init__(self):
        ...
        # ---- Load samples (raw, keep raw units; offsets are applied dynamically)
        self.samples_raw = self._load_samples_raw(self.csv_path, self.person_id)
        if len(self.samples_raw) < 2:
            raise RuntimeError(f"Not enough samples for person_id='{self.person_id}' in {self.csv_path}")
        self.get_logger().info(f"Loaded {len(self.samples_raw)} samples for person_id='{self.person_id}'")

        # ---- Runtime (dynamic) parameters cache
        self._read_dynamic_params()
        ...
        # Apply initial reset (spawn at first sample using current offsets)
        self._reset_playback()

        # ---- Service: /human_replay/reset
        self.reset_srv = self.create_service(Trigger, "~/reset", self._on_reset)

        # ---- Live param update callback
        ...
        self.get_logger().info("Ready. Use: ros2 param set /human_replay x_offset ... then ros2 service call /human_replay/reset std_srvs/srv/Trigger")

    # -------------------------
    # Params
    # -------------------------
    def _read_dynamic_params(self):
    ...
    def _on_params(self, params):
        # Accept changes; do not auto-reset (you control reset via service)
        for p in params:
            if p.name in ("scale", "x_offset", "y_offset", "max_step",
                          "min_speed_for_yaw", "yaw_noise_std", "yaw_smoothing"):
                pass
        # Update cache now
        self._read_dynamic_params()
    ...
    # Data load
    # -------------------------
    def _load_samples_raw(self, csv_path: str, person_id: str):
        rows = []
        ...
        # store raw and normalized time
        samples = []
        for (ts, x_raw, y_raw, z_raw) in rows:
            t = (ts - t0).total_seconds()
            samples.append((t, x_raw, y_raw, z_raw))
        return samples

    def _sample_to_map(self, t: float, x_raw: float, y_raw: float, z_raw: float) -> Sample:
        x = x_raw * self.scale + self.x_offset
        y = y_raw * self.scale + self.y_offset
        z = z_raw * self.scale
        return Sample(t=t, x=x, y=y, z=z)

    def _get_sample(self, idx: int) -> Sample:
        t, xr, yr, zr = self.samples_raw[idx]
        return self._sample_to_map(t, xr, yr, zr)

    def _interpolate(self, t_play: float) -> Tuple[float, float]:
        while (self.i + 1) < len(self.samples_raw) and self.samples_raw[self.i + 1][0] < t_play:
            self.i += 1
        i2 = min(self.i + 1, len(self.samples_raw) - 1)

        s1 = self._get_sample(self.i)
        s2 = self._get_sample(i2)
    ...
    def _reset_playback(self):
        self._read_dynamic_params()  # make sure newest params are used
        self.start_time_ros = self.get_clock().now()
        self.i = 0
        s0 = self._get_sample(0)
        ...
        if self.map_grid:
            ...
            if changed:
                self.get_logger().warn(f"Reset spawn was not free -> snapped to ({xn:.3f},{yn:.3f})")
                self.last_x, self.last_y = xn, yn

        self.get_logger().info(
            f"RESET OK: offsets=({self.x_offset:.3f},{self.y_offset:.3f}) "
            f"yaw_noise_std={self.yaw_noise_std:.3f} yaw_smoothing={self.yaw_smoothing:.3f} "
            f"spawn=({self.last_x:.3f},{self.last_y:.3f})"
        )

    def _on_reset(self, req, res):
        self._reset_playback()
        res.success = True
        res.message = "human_replay reset done"
        return res
